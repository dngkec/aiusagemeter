...
`test-windows.ps1` launches the Microsoft.Testing.Platform executable directly. `dotnet test`
wraps that same executable, but its .NET 10 SDK server mode reports `Zero tests ran` with exit
